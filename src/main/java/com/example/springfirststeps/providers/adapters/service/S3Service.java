package com.example.springfirststeps.providers.adapters.service;
...
public class S3Service {
    ...
    public void putObject(String bucketName, MultipartFile representation, boolean publicObject) {
        String nombreArchivo = UUID.randomUUID().toString().concat("-").concat(representation.getOriginalFilename());
        try {
            File convFile = new File(representation.getOriginalFilename());
            convFile.createNewFile();
            FileOutputStream fos = new FileOutputStream(convFile);
            fos.write(representation.getBytes());
            fos.close();
            if (publicObject) {
                var putObjectRequest = new PutObjectRequest(bucketName, nombreArchivo, convFile).withCannedAcl(CannedAccessControlList.PublicRead);
                amazonS3Client.putObject(putObjectRequest);
            } else {
                var putObjectRequest = new PutObjectRequest(bucketName, nombreArchivo, convFile).withCannedAcl(CannedAccessControlList.Private);
                amazonS3Client.putObject(putObjectRequest);
            }
        } catch (Exception e) {
